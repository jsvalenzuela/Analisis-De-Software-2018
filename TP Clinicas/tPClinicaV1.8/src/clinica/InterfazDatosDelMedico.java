...
public class InterfazDatosDelMedico extends JFrame{
	...
	public InterfazDatosDelMedico() {
		...
		lblFondoDatosDelMedico.setIcon(new ImageIcon(Login.class.getResource("borde2.png")));
			...
			public void actionPerformed(ActionEvent e) {
				...
				else
				{
					if (texMatricula.getText().length()*texNombre.getText().length()*texEspecialidades.getText().length()*texFechaDeIngreso.getText().length()*texTelefono.getText().length() != 0) {
						...
						medico = new Medico(ultimoCod, texMatricula.getText(), texNombre.getText(), enfermedades, texFechaDeIngreso.getText(), texTelefono.getText());
				        ...
				        try
				        {
				            fichero = new FileWriter("./bin\\Medicos.txt", true); //Con pasarle por parametro el true agrega al final sin borrar el contenido
				            pw = new PrintWriter(fichero);
				            
				            pw.println(encriptar(medico.toString()));
		...
		lblEspecialidades = new JLabel("Especialidades: (separadas por un unico espacio)");
		...
		lblEspecialidades.setBounds(10, 336, 390, 26);
		...
		lblFechaDeIngreso.setBounds(1, 287, 267, 26);
		...
		texFechaDeIngreso.setBounds(278, 293, 132, 20);
