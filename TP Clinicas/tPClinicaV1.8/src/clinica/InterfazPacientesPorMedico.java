...
public class InterfazPacientesPorMedico extends JFrame {
	...
	public InterfazPacientesPorMedico() {
		...
		lblListadoPacientes.setIcon(new ImageIcon(Login.class.getResource("borde2.png")));
					...
					{	
						...
						if (!errorArchivoDiagnosticos) {
							...
							try {
								...
								int codigoPacienteArrayList = -1;
								
								if (codPacientes.size() != 0) {
								...
								if (codPacientes.size() != 0) {
									...
									JOptionPane.showMessageDialog(rootPane, "El medico no atendio a ningun paciente", "", JOptionPane.INFORMATION_MESSAGE, null);
